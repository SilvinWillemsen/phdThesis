...
Lastly, it has been found that when $|\up_{(l,m)}^n| \lessapprox 10^{-306}$ for any coordinate $(l,m)$ (which happens when the body has not been collided with for a prolonged period of time), the CPU usage increases considerably. This could be explained by the fact that calculations with values close to the smallest value that the computer can describe, are handled differently by the application.  %\SWcomment[Maybe we should ask Craig about this..] 

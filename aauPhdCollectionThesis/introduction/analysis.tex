...
\begin{align}
    \langle f, \dxx g\rangle = \langle \delta_{xx}f, g \rangle_d+f_N\delta_{x+}g_N-g_N\delta_{x+}f_N -f_0\delta_{x-}g_0+g_0\delta_{x-}f_0,
\end{align}
or, using the reduced domains,
\begin{align}
    \langle f, \dxx g\rangle = \langle \delta_{xx}f, g \rangle_{\underline{\overline{d}}}+f_N\dxp g_N-g_N\dxm f_N -f_0\dxm g_0+g_0\dxp f_0.
\end{align}
...
Rather than the roots of a characteristic equation needing to be bound by unity, to arrive at a condition the energy must be \textit{positive definite}. It can be shown that an equation of the form 
...
Equation \eqref{eq:quadraticForm} can be used to prove stability for the mass spring system using the energy balance in Eq. \eqref{eq:energyBalanceMassSpring}. 
 easily conclude that $\t$ is positive definite due to non-negative $M$ and the fact that $\dtm \un$ is squared. The potential energy $\v$, however, is of indefinite sign. Expanding the operators in Eq. \eqref{eq:energyBalanceMassSpring} yields
\begin{align*}
...
\end{equation*}
and for $\h$ to be positive definite, i.e., $\h > 0$ the following condition must hold
\begin{equation*}
...
For the 1D wave equation, one can take the energy balance in Eq. \eqref{eq:energyBalance1DWave} and conclude that $\t$ is positive definite due to the non-negativity of the parameters and $(\dtm \uln)$ being squared. The potential energy, however, is of indefinite sign. One can rewrite $\v$ using identity \eqref{eq:prodIdentityEnergyStab} as
\begin{align*}
...
\end{align*}
One can then use the following bound \cite{theBible}
...
\end{align*}
Filling this condition into the energy balance in Eq. \eqref{eq:energyBalance1DWave} yields
\begin{align*}

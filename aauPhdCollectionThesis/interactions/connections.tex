...
\SWcomment[might want to remove this entire section.. ]
...
As the schemes are connected, the matrix form is slightly more involved.
...
This section shows an example of an implementation of the string-plate system. The \texttt{MATLAB} code of the implementation can be found online\footnote{\url{https://gist.github.com/SilvinWillemsen/91d10f6279af7fd0da7fb65fbd7647da}} The parameters for the stiff string can be found in Table \ref{tab:stiffStringParams} (with $L = 1.5$ m and $T = 555$ N) and for the thin plate in Table \ref{tab:thinPlateParams} (with $H=5\cdot10^{-4}$). Both systems have simply supported boundary conditions. Additional parameters used for the connection are
\begin{equation*}
    K_1 = 10^4\ \,\text{N/m}, \quad K_3 = 10^7\ \,\text{N/m}^3, \qaq R = 10\ \,\text{kg/s}.
\end{equation*}
Figure \ref{fig:stringPlate} shows an visualisation of the string plate system excited with a raised cosine. 
\begin{figure}[h]
...
When implementing connections (or any other interactions for that matter), one mostly does the following:
\begin{enumerate}
    \item Calculate intermediate scheme, i.e., without force terms: $\u^\text{I}$ and $\w^\text{I}$
    \item Calculate $u^\star$ and $w^\star$: $\i_u \u^\text{I}$ and $\i_w \w^\text{I}$
    \item Calculate force $f^n$
    \item Add force term to intermediate scheme $\u^{n+1} = \u^{\text{I}} - \j_u f^n$ and $\w^{n+1} = \w^{\text{I}} + \j_w f^n$
\end{enumerate}
\SWcomment[not done]


\subsection{Energy analysis}
\SWcomment[still needs stuff]
\begin{equation}

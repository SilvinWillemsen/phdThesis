...
This chapter provides an extended summary for the physical model used in the papers ``Real-time Implementation of a Physical Model of the Tromba Marina'' \citeP[D] and ``Resurrecting the Tromba Marina: A Bowed Virtual Reality Instrument using Haptic Feedback and Accurate Physical Modelling'' \citeP[E]. After a brief introduction of the tromba marina, and summarising the physical model, with reference to the theory explained in the previous parts of this thesis, this chapter elaborates on the contents of the above papers by providing more details on the implementation. As much as possible, this chapter follows the notation of paper \citeP[D], as long as it is coherent with what has already been presented in this thesis. Discrepancies between this chapter and the paper will be clearly highlighted. 
...
\SWcomment[Insert figures of the tromba and `shoe' here, like in the paper]
...
To simplify things slightly, and looking towards real-time implementation, one could instead solve for $\delta_{t\cdot}\eta_\sm^n$ and $\delta_{t\cdot}\eta_\mp^n$ directly, which would require a $2\times 2$ matrix inversion each iteration.
...
\begin{align*}
    ...
    \zbr^\star & = \frac{2\zbr^n-\zbr^{n-1}-\kappa_\ptxt^2k^2\delta_{\Delta\boxplus}\delta_{\Delta\boxplus}\zbr^n+\sigma_{0,\text{p}}k\zbr^{n-1}+ 2\sigma_{1,\text{p}}k^2\delta_{t-}\delta_{xx}\zbr^n}{1+\sigma_{0,\text{p}}k}.
\end{align*}
Here, $c = \sqrt{T/\rho_\stxt A}$, $\kappa_\stxt = \sqrt{E_\stxt I /\rho_\stxt A}$ and $\kappa_\ptxt = \sqrt{D / \rho_\ptxt H}$.  
...
\begin{equation*}
    \h_\ptxt = \frac{\rho_\ptxt H}{2} \left\lVert\delta_{t-}\zlmn\right\rVert_{d_\ptxt}^2 + \frac{D}{2}\langle\delta_{\Delta\boxplus} \zlmn, e_{t-}\delta_{\Delta\boxplus} \zlmn\rangle_{\overline{\underline{d_\ptxt}}}\ .
\end{equation*}

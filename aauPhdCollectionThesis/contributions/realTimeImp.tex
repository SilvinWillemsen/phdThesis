...
\section{MATLAB versus C++}
It is usually a good idea to prototype a physical modelling application in \texttt{MATLAB} for several reasons:
\begin{itemize}
...
After a long period of time, which depends on the damping coefficients, state values can get in the range of \textApprox$10^{-307}$! Numbers in this range are referred to as \textit{denormalised numbers} and operations with these are ``extremely slow'' \cite{CPPdenormalised}.

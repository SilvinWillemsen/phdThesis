...
The trombone is an extremely interesting instrument from a simulation perspective due to the time-varying length of the acoustic tube. In FDTD-based simulations the trombone poses a challenge due to issues regarding grid points and simulation quality (as also pointed out by Harrison in \cite{Harrison2018}). As an attempt at resolving these issues has been made by the dynamic grid method presented in Chapter \ref{ch:dynamicGrid} and paper \citeP[G], the trombone makes a application for this method.
...
After providing a short summary of paper  \citeP[H] that relates the paper to the rest of this thesis, this chapter extends the paper by providing implementation details on the lip reed and design considerations for the dynamic grid. 
...
A real-time simulation was created and implemented in C++ using the JUCE framework (see Chapter \ref{ch:realtime}). The application and a demo can be found online.\footnote{\url{https://github.com/SilvinWillemsen/cppBrass/releases/}}\textsuperscript{,}\footnote{\url{https://youtu.be/Ht5gVNrshYo}} The total range of the tube length ($2.593$-$3.653$ m) could be traversed in $0.06$ s (corresponding to 20 samples between grid configurations), and an informal evaluation by the authors showed that no noticeable artefacts were observed. Naturally, proper listening tests need to confirm this It must be noted that to achieve this high rate of change between grid configurations, the Courant number has been set slightly lower than the stability condition ($\lambda = 0.999$). This decrease in $\lambda$ does cause a small, but negligible decrease in simulation quality and bandwidth (see Section \ref{sec:quality1DWave}). 

The main contribution of the paper, is the use of the dynamic grid method to implement the time-varying acoustic tube. Several design considerations made during the process will be presented below. 

\section{Design considerations}
The main challenge in this contribution was to apply the dynamic grid method to a first-order system of PDEs. It was decided to apply the method to the pressure grid 

Another difference was that the length $L$ rather than the wave speed $c$ was made to be time-varying, but as paper \citeP[G]

a change in length relates to a change in wavespeed according to Eq. \eqref{eq:fundamentalFreq} 

1D wave equation (which is what the acoustic tube is eventually based on) 

As paper \citeP[H] provides extensive details on the implementation of the method in this context, here, only some design considerations\todo{regarding location of the adding and removing..} will be discussed.

\subsubsection{Design considerations}
As the geometry varies it matters a lot where points are added and removed as this might influence the way that the method is implemented. The middle of the slide crook was chosen, for several reasons. First of all, it could be reasonable to assume that the air in the tube to ``go away from'' or ``go towards'' that point as the slide is extended or contracted. Secondly, the geometry is constant and because the geometry does not vary there. Experiments with adding / removing grid points where the geometry varies have been left for future work. \SWcomment[even more speculative.. $\rightarrow$] It could be argued that it makes more sense to add points at the ends of the inner slides as ``tube material'' is also added there. This would mean that the system should be split in three parts: ``inner slide", ``outer slide" and ``rest", and would complicate things even more.

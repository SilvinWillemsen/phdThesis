...
Apart from being potentially sonically interesting, dynamic parameter changes also happen in the real world. A prime example is the trombone published in paper \citeP[H] using the method presented in paper \citeP[G]. A collection of examples for use cases of the method are listed in Section \ref{sec:examples}.
...
\begin{equation*}\label{eq:quadMat}
    ...
    \end{bmatrix}
\end{equation*}
which, as $\lambda = 1$ at all times, can be simplified to
\begin{equation}\label{eq:quadMat}
...
\begin{equation}\label{eq:oneStepFormDynamicGrid}
    ...
    \end{bmatrix},
\end{equation}
where $\mathbf{0}$ is a $\fullMn\times \fullMn$ matrix of zeros. Notice that, in the definitions for $\A^n$ and $\C^n$, $\beta$ can go to infinity, when $\alpha = \varepsilon = 0$ through Eq. \eqref{eq:betaDef}. In this case, $\varepsilon$ has to be set to a tiny non-zero value to avoid undefined results. 

The modal analysis can be used to analyse the system while parameters are varied to determine whether the behaviour of the system is satisfactory. Figure \ref{fig:modalAnalysisOneStep} shows the result of a modal analysis performed on the one-step form shown in Eq. \eqref{eq:oneStepFormDynamicGrid} according to Section \ref{sec:oneStepForm}. The inner boundaries have been placed as close to the right boundary as possible, such that the number of intervals $M^n$ and $M_w^n$ are calculated using Eq. \eqref{eq:alternativeM}. The wave speed is varied linearly between $c^0 = 2940$, corresponding to $\Nfrac^0 = 15$, and $c^{n_\text{end}} = 2205$ corresponding to $\Nfrac^{n_\text{end}} = 20$. 
...
More results can be found in paper \citeP[G]. 
...
\begin{figure}[h]
    \centering
    \subfloat[Points are added in middle.]{\label{fig:linearModesAddInCenter}{ \includegraphics[width=0.49\textwidth]{figures/contributions/dynamicgrid/interpolation/linearMid.eps}}}
    \hfill
    \subfloat[Points are added at close to the boundary.]{\label{fig:linearModesAddBoundary}{ \includegraphics[width=0.49\textwidth]{figures/contributions/dynamicgrid/interpolation/linear1.eps}}}
...
\begin{figure}[h]
    \centering
    \subfloat[Points are added in middle.]{\label{fig:quadModesAddInCenter}{ \includegraphics[width=0.49\textwidth]{figures/contributions/dynamicgrid/interpolation/quadMid.eps}}}
    \hfill
    \subfloat[Points are added at close to the boundary.]{\label{fig:quadModesAddBoundary}{ \includegraphics[width=0.49\textwidth]{figures/contributions/dynamicgrid/interpolation/quad1.eps}}}
...
If Eq. \eqref{eq:alternativeM} is used, and grid points are added close to the right boundary, the Dirichlet boundary condition at this boundary needs to be extended to be simply supported such that $w_2^n = -w_0^n$. As the value at the boundary remains $0$, i.e., $w_1 = 0$, this alters Eqs. \eqref{eq:cubicVirtualGridpoints} to be
\begin{align*}
...
\begin{figure}[h]
    \centering
    \subfloat[Points are added in middle.]{\label{fig:cubicModesAddInCenter}{ \includegraphics[width=0.49\textwidth]{figures/contributions/dynamicgrid/interpolation/cubicMid.eps}}}
    \hfill
    \subfloat[Points are added close to the boundary.]{\label{fig:cubicModesAddBoundary}{ \includegraphics[width=0.49\textwidth]{figures/contributions/dynamicgrid/interpolation/cubic1.eps}}}
...
\begin{figure}[h]
    \centering
    \subfloat[Points are added in middle.]{\label{fig:quarticModesAddInCenter}{ \includegraphics[width=0.49\textwidth]{figures/contributions/dynamicgrid/interpolation/quarticMid.eps}}}
    \hfill
    \subfloat[Points are added at close to the boundary.]{\label{fig:cubicModesAddBoundary}{ \includegraphics[width=0.49\textwidth]{figures/contributions/dynamicgrid/interpolation/quartic1.eps}}}
...
The hammered dulcimer is another example where the strings are tensioned over a bridge where one can play the string at one side of the bridge, while pushing down on the same string on the other side.\footnote{Amazing Hammered Dulcimer Musician - Joshua Messick: \url{https://youtu.be/veuGTnzgNRU?t=215}}

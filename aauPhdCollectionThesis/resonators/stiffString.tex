...
\end{equation*}
The update equations for the boundary points will not be given here. Instead the matrix form the scheme will be provided below. 
...
There exists a formula to calculate the loss coefficients $\sz$ and $\so$ from $T_{60}$ values at different frequencies (see \cite[Eq (7.29)]{theBible}). During this project, however, these values have been tuned by ear and are usually put on 
...
\begin{table}[h]
    ...
    \end{tabular}
    \caption{Parameters and the values most commonly used over the course of this project.\label{tab:zIdentities}}
    \end{center}
\end{table}
...
Apart from the obvious material properties such as density, stiffness and geometry, the sound surprisingly much determined by $\so$! For low values the output can be extremely metallic, whereas for high values the
...
\end{gather}
Rewriting this to the form in \eqref{eq:polynomialForm}, and using $\S = \sin^2(\beta h /2)$ for brevity, yields
\begin{equation*}
...
Comparing the energy balance in Eq. \eqref{eq:energyBalanceStiffString} to the energy balance for the 1D wave equation in Eq. \eqref{eq:energyBalance1DWave}, one can observe that the balances are identical, apart from the second term in the definition for $\v$ in Eq. \eqref{eq:energyBalanceStiffString}. 
...
This section follows the process for modal analysis of a scheme in one-step form described in Section \ref{sec:oneStepForm}.

To be able to perform a modal analysis on the FD scheme in \eqref{eq:stiffStringFDS}, it must be written in one-step form due to the damping present in the system (see Section \ref{sec:oneStepForm}). Using with the matrix form in \eqref{eq:matrixFormStiffString}, the damped stiff string can be written as
...
Assuming test solutions of the form $\w = z\boldPhi$, and recalling that $z=e^{sk}$ and complex frequency $s = j\omega + \sigma$, we get the following eigenvalue problem (see Section \ref{sec:eigenValueProblems})
\begin{equation}
    z\boldPhi = \Q \boldPhi,
\end{equation}
...
\end{equation}
The (angular) frequency of the $p$\th mode can then be obtained using $\mathfrak{I}(s_p)$ and the damping per mode as $\mathfrak{R}(s_p)$. Plots of modal frequencies and their damping coefficients have been plotted for different parameter values


As the values of $\Q$ are real, the solutions come in complex conjugates. Only the modes with a positive frequency have been chosen frequencies of interest are the positive

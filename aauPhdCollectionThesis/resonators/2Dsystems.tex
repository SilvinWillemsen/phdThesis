...
This chapter starts by providing some additional information about 2D grid functions and operators. Then, the 2D wave equation is presented, which is used to extend the analysis techniques presented in Chapter \ref{ch:analysis} to 2D. Afterwards, two 2D models that have been used in this project, the thin plate and the stiff membrane, will be described in a similar fashion. %Section \ref{sec:radialCoordinates} briefly elaborates on radial coordinate systems and their shortcomings. 
...
\end{equation}
Using these shift operators, a discrete approximation of the Laplacian in Eq. \eqref{eq:laplacian} can be made\footnote{Notice that the $\dDelta$ operator is identical to $\delta_{\Delta \boxplus}$ in \cite{theBible}. The former will be used as it is more compact, and it is the only discretisation to the Laplacian used in this work.} 
...
%     \end{aligned}
% \end{equation}
Also see Figure \ref{fig:biharmonic}.
\begin{figure}[h]
    \centering
...
\end{equation}
for the scheme to be stable. See Section \ref{sec:stability2Dwave} for a derivation of this. Writing this condition in terms of the grid spacing, places the following condition on $h$:
...
\begin{equation}
    ...
    \end{bmatrix},
\end{equation}
and could be used to make a `for-loop implementation' of the update equation. This could indeed be the strategy if one would implement the scheme in e.g. C++ (see Chapter \ref{ch:realtime}). For a more compact implementation in \texttt{MATLAB}, however, one could `stack' or `flatten' the state matrices to vectors and update the scheme using matrix-vector multiplication (as done for the stiff string in Section \ref{sec:implementationStiffString} for example). Again using Dirichlet boundary conditions, the stacked state vector will be structured as
...
\subsubsection{Step 4: Implementation}
Figure \ref{fig:energyThinPlate} shows the energetic output of an implementation of the thin plate. Notice that the damping present in the system causes $\h$ to decrease.
\begin{figure}[h]
    \centering
    \begin{tikzpicture}[->,node distance=3cm,
        thick,main node/.style={circle,draw}]
        ...
        \node[] (t) at (-5.75, 0) {\small $\color{blue}\mathfrak{t}$};
      \end{tikzpicture}
      \caption{The kinetic (blue), potential (red), and total (black) energy of an implementation of the thin plate are plotted in the left panel. The right panel shows the normalised energy (according to Eq. \eqref{eq:normalisedEnergyDamping}) and shows that the deviation of the energy is within machine precision. \label{fig:energyThinPlate}}
\end{figure}
...
\end{equation}
where the parameters are identical to those in Eqs. \eqref{eq:2DwavePDE} (with $c = \sqrt{T/\rho H}$) and \eqref{eq:platePDE}.
...
Following familiar techniques from Sections \ref{sec:stabilityAnalysis} and \ref{sec:stability2Dwave}, the characteristic equation of the FD scheme in Eq. \eqref{eq:stiffMembraneFDScompact} can be obtained:
\begin{equation}
    \begin{aligned}
...
\end{equation*}
Finally, recalling the definitions for $\lambda$ and $\mu$ from Eq. \eqref{eq:lambdaMuStiffMembrane}, this can be rewritten in terms of $h$ to get the following stability condition for the stiff membrane:\footnote{This stability condition was wrong in paper \citeP[F]. It has been corrected here and included in Appendix \ref{app:paperErrata}.}  
\begin{equation}

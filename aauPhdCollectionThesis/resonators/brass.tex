...
\end{align}
\end{subequations}
See Appendix \ref{app:websterDerivations} for a derivation of this. The boundary term $\b_\text{r}$ can be decomposed in $\h_\text{b}$ and $\q_\text{b}$ in used in Eq. \eqref{eq:powerBalanceWebster}. These can be obtained substituting the definition of the radiating boundary Eq. \eqref{eq:centRadBound} into \eqref{eq:centStrictDissipRight} to get
\begin{align*}
...
\begin{equation}
    [S] \delta_{tt}\Psi^n_l = c^2\dxp(\Sm(\delta_{x-}\Psiln)),
\end{equation}
where $[S]$ is a second-order approximation to the true geometry of the acoustic tube.

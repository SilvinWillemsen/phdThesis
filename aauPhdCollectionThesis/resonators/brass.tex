...
\end{align}
The input signal is arbitrary, but looking towards lip excitation, and following \cite{theBible}, one can set the input to a pulse train as shown in Figure \ref{fig:inputWebster}. More details can be found in Section \ref{sec:pulseTrain}.
\begin{figure}[h]
    \centering
...
\def\r{\text{r}}
\def\one{{(1)}}
As done in Section \ref{sec:webstersEq}, radiation can be added to the acoustic tube. The radiation model used in this work was proposed by Levine and Schwinger in \cite{Levine1948}, its discretisation proposed by Silva et al. \cite{Silva2009}. Although other radiation models exist, state-of-the-art work by Bilbao and Harrison on brass instruments based on the first-order system in \eqref{eq:firstOrderSystem} also use this model \cite{Bilbao2013, Harrison2018}. See Figure \ref{fig:circuit} for a circuit representation of the Levine and Schwinger radiation model.\footnote{The Levine and Schwinger model has been taken from \cite{Harrison2018} `as is' and this thesis will not provide additional background on the working of the model. The interested reader is referred to \cite{Harrison2018} for more information.}
...
\begin{subequations}
    ...
    \end{align}
\end{subequations}
The various parameters are taken from \cite{Harrison2018} and can be found in Table \ref{tab:radiationParams}
\todo{temperature dependent parameters somewhere?}
\todo{table}
\subsection{Energy analysis}
This section shows the result of the energy analysis of the above radiation model. The full derivation will not be given here, but can be found in \cite{Harrison2018}. The result is included in this thesis for completeness, and such that it cam be used for implementation and debugging purposes.
...
\end{equation}
which, using Eq. \eqref{eq:barVars}, one can rewrite this to
\begin{equation}
    \mathfrak{b}_\r = \bar p\bar S_N\bar v.
\end{equation}
Following the derivation in \cite[Sec. 4.1.4]{Harrison2018}, this can eventually be rewritten to
\begin{equation}

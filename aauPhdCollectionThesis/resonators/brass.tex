...
The next step is to find definitions for $\el$ and $\er$ such that the boundaries are strictly dissipative. In other words, the boundary terms need to be rewritten such that 
\begin{align*}
...
\end{equation}
and making this solely dependent on the known values $v_\one^n$, $p_\one^n$ and $p_N^n$ and the unknown $p_N^{n+1}$ (as this can be obtained using \eqref{eq:preSolutP}).

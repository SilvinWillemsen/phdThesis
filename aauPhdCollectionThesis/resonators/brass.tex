...
The interesting thing about using a variable cross-section, is that it causes the system to exhibit dispersive behaviour, especially at locations of high (spatial) variation of $S$. See Figure \ref{fig:websterPropagation}. Contrary to frequency dispersion as happens in a stiff string (see Chapter \ref{ch:stiffString}), all frequencies travel at the same speed, but some components of the wave get reflected in different ways, depending on the geometry.
...
\begin{figure}[h]
    ...
    \caption{Wave propagation and dispersion in an acoustic tube of varying cross section modelled by Webster's equation in Eq. \eqref{eq:webstersPDE}. The acoustic potential $\Psi$ is plotted as well as  red for positive and blue for negative values of $\Psi$.  \label{fig:websterPropagation}}
\end{figure}
...
\begin{figure}[h]
    ...
    \captionof{figure}{A pulse train with a frequency of $213$ Hz. This is used to excite the implementation of Webster's equation in Section \ref{sec:outputWebster}. \label{fig:inputWebster}}
\end{figure}
...
\begin{figure}[h]
    ...
    \captionof{figure}{The geometry used for the implementation. See paper \citeP[H] for more details. \label{fig:geometryWebster}}
\end{figure}
...
Energy analysis of Webster's equation with a radiating end might seem straightforward. However, due to the varying cross-sectional area, the energy balance deserves a more detailed treatment, especially the boundaries. For this analysis, (centred) Neumann boundary conditions are used for both boundaries and the input is ignored.% to retain generality. %These can then easily be adapted to Dirichlet if needed.
...
Then using identity Eq. \eqref{eq:prodIdentity2} and identity \eqref{eq:prodIdentity1} for the first term in Eq. \eqref{eq:powerBalanceWebster} yields 
\begin{equation*}
...
As (centred) Neumann conditions are used, the goal is to find definitions for $\el$ and $\er$ such that the boundaries are strictly dissipative. In other words, the boundary terms need to be be rewritten such that 
\begin{align*}
...
Figure \ref{fig:energyWebsters} shows the energetic output of Webster's equation with a radiating boundary at $x=L$. To highlight the effect of the radiation on the energy the parameters are set to $L = 1$ and $S(x) = 0.01$ for $x\in \D$.  The system is excited with a raised cosine close to the left boundary, and when the excitation reaches the radiating boundary, the total energy in the system decreases due to the losses. The energy stored by the boundary is also shown and indeed increases when the wave reaches the boundary.
\begin{figure}[h]
    \centering
...
\end{equation}
where $\phi_l > 0$ is defined over the same domain as $u$. The following condition can then be put on $\v$
\begin{align*}
...
If the tube is perfectly cylindrical and thus $S(x)$ is constant, Webster's equation reduces to the 1D wave equation. If the inertia coefficient of the radiating boundary $a_2$
